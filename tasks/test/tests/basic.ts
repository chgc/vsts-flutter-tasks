...
assertDirectory(
  (process.env['AGENT_HOMEDIRECTORY'] = path.join(agentPath, 'home'))
);
assertDirectory(
  (process.env['AGENT_TOOLSDIRECTORY'] = path.join(agentPath, 'tools'))
);
assertDirectory(
  (process.env['AGENT_TEMPDIRECTORY'] = path.join(agentPath, 'temp'))
);
assertDirectory(
  (process.env['AGENT_BUILDDIRECTORY'] = path.join(agentPath, 'build'))
);

// Run install tests
process.env['FlutterToolPath'] = path.join(
  agentPath,
  'tools',
  'Flutter',
  '0.8.2-beta',
