...
async function main(): Promise<void> {
  ...
  var flutterPath =
    task.getVariable(FLUTTER_TOOL_PATH_ENV_VAR) ||
    process.env[FLUTTER_TOOL_PATH_ENV_VAR];
  flutterPath = path.join(flutterPath, 'flutter');
  if (!flutterPath) {
    throw new Error(
      `The '${FLUTTER_TOOL_PATH_ENV_VAR}' environment variable must be set before using this task (you can use 'flutterinstall' task).`
    );
  }
